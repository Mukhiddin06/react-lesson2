...
function App() {
  ...
  return (
    <>
    {/* task- 3 */}
    <button className='btn' onClick={addCounter}>add counter</button>
    
    {
      counter?.map((item, index)=>{
        return (
        <div key={index} className="button-wrapper">
          <button className='btn' onClick={() => increaseCount(index)}>+</button>
          <span>{item}</span>
          <button className='btn' onClick={() => decreaseCount(index)}>-</button>
        </div>
        )
      })
    }
    ...
    <button onClick={()=> setActive(!active)}>{active ? 'Login' : 'Logout'}</button>
    {/* task -2 */}
    {/* task -1  */}
      <p>{metr} metr</p>
